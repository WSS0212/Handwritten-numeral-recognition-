...
for file in fileList:
    img_test = cv2.imread("./test/" + file,cv2.IMREAD_GRAYSCALE)
    img_resize = cv2.resize(img_test,(28,28),interpolation=cv2.INTER_CUBIC)
    img_Binarization = cv2.adaptiveThreshold(img_resize, 255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY, 5, 10) #上面我们将b设置为5，常数设置为10。

    # 图像颜色反转
    img_info=img_Binarization.shape
    image_height=img_info[0]
    image_weight=img_info[1]
    img_dst=np.zeros((image_height,image_weight,1),np.uint8)
    for i in range(image_height):
        for j in range(image_weight):
            grayPixel=img_Binarization[i][j]
            img_dst[i][j]=255-grayPixel

    # 图像转成tensor
    transf = transforms.ToTensor()
    img_tensor = transf(img_dst)
    img_tensor = torch.unsqueeze(img_tensor, dim=1) # 增加维度
    inputs = img_tensor
    
    # 输入到模型进行识别
    test_output = cnn(inputs)
    pred_y = torch.max(test_output, 1)[1].data.numpy()
    print('预测结果：' , int(pred_y))  # 打印识别后的数字
    ...
    # mean = [0.5, 0.5, 0.5]
    # img = img * std + mean
    cv2.imshow('待预测数字', img_test)  # opencv显示需要识别的数据图片
